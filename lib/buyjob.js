			...
			if(err || post == null) {
	            ...
	            if(currentAddress) {
			        ...
			        client.getaddressbalance(currentAddress, function (err, balance) {
				        ...
				        console.log('Checking if user has enough money');
	            			if(balance >= price || provideService) {
							...
							console.log("Transfer asset response: " + response);
	            					if(err) {
	            						//Error in transferring LabCoins for purchasing job
	            						jobpostListClient.internaljoblists(req, res, 1, 'Error in transferring LabCoins for the job.');
								...
								console.log("Updainging buyer info. username = " + username);
									...
									console.log ("Balance before update = " + user.local.balance);
		        	            			...
		        	            			user.save(function(err) {
		        	    	            		...
		        	    	            		var msg = 'You bought the item successfully! You currently have '+ user.local.balance + ' LabCoins.';
		        	    	            		req.user = user;
								...
								var messageClient = require("../lib/sendMessageToSeller");
                                                                if (provideService) {
                                                                        messageClient.sendMessage("You agreed to provide item " + post.title + " to " + sellerUsername + ". Email the requester to provide your item. Your current balance is " + user.local.balance + " LabCoins.", sellerAddress);

                                                                } else {
                                                                        messageClient.sendMessage("You have bought item " + post.title + " from seller " + sellerUsername + ". Email the seller to get what you bought. Your current balance is " + user.local.balance + " LabCoins.", currentAddress);
	            						...
	            						User.findOne({ 'local.username' :  sellerUsername }, 
		        	            			...
		        	            			var messageClient = require("../lib/sendMessageToSeller");
								if (provideService) {
    	            			            			messageClient.sendMessage(username + " has agreed to provide item " + post.title + ". Your current balance is " + selleruser.local.balance + " LabCoins. Email provider to claim your item.", currentAddress);		        	            			

								} else {
    	            			            			messageClient.sendMessage(username + " has bought the item " + post.title + ". Email the buyer to fulfill the service. Your current balance is " + selleruser.local.balance + " LabCoins. ", sellerAddress);		        	            			
