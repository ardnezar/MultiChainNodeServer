	            				...
	            				transferClient.transferInternal(currentAddress, sellerAddress, post.price, function (err) {
	            					if(err) {
	            						...
	            						User.findOne({ 'local.username' :  username }, 
	            						          function(err, user) {
		            						user.local.balance = balance1;	            								    	    					    	  
		        	            			console.log('Updating buyer balance:'+balance);
		        	            			user.save(function(err) {
		        	    	            		...
		        	    	            		var msg = 'You bought the job successfully. Your currently have '+ balance1 + ' dicoins.';
		        	    	            		req.user = user;
	            			            ...
	            			            LeaderBoard.findOne({ 'username' :  sellerUsername }, 
	            			            		function(err, seller) {
			            			            	if(seller) {
			            			            		...
			            			            		var currentsellerbalance = seller.balance+price;
			    	            			            messageClient.sendMessage("You have sold the job "+post.title+" to "+username+".Your current balance is "+currentsellerbalance+".",sellerAddress);
		        			            			} else {
		        			            				User.findOne({ 'local.username' :  sellerUsername }, 
	            			            						function(err, user) {	
	            			            					var newLeader = new LeaderBoard();
