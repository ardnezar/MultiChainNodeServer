	            ...
	            if(user.local.address) {
	            	...
	            	client.getaddressbalance(user.local.address, function (err, balance) {
			        	if(err) {
	            			...
	            			var newMessage = '';
	            			if(balance === 0 && newUser.local.validated !== true) {
	            				newMessage = 'Account validation in progress. Your account will be updated with your initial 100 LabCoins shortly.';
	            				newUser.local.validated = true;
	            				res.render('accounts.ejs', {	            					
					            	message: newMessage,
					                user : req.user, // get the user out of session and pass to template,
					                flag: false
