    				    	...
    				    	if (err) {			    		
    				    		...
    				    		if(resp.error) {
    				    			...
    				    			client1.getaddressbalance(userFrom.local.address, function (err, balance) {
    						       		if(balance) {	        				        			
    							    		//Update balance
    						       				userFrom.local.balance = balance[0].qty;
...
//		    console.log("Transfer req body:"+JSON.stringify(req.body));
		    ...
		    if(from === null || to === null || asset === null || qty === null) {
		    	cb('Invalid transfer request.'); 
		    } else {
		    	User.findOne({'local.username':  from }, function(err1, userFrom) {
		    		if(err1) {
		    			cb(err1); 		    			
		    		} else {
		    			//Buyer address valid
		    			console.log("Buyer Address:"+userFrom.local.address);
		    			User.findOne({'local.username':  to }, function(err, userTo) {
		    	    		if(err) {
		    	    			cb(err); 	
		    	    		} else if(userTo) {
		    	    			//Valid Recipient user, get the actual address first		    	    			
		    	    			console.log("Recipient Address:"+userTo.local.address);
		    				    var arr = [];
//		    				    var fromAddr = userFrom.local.address.replace(/"/g, '');
//		    				    var toAddr = userTo.local.address.replace(/"/g, '');
...
//		    						    	});
		    	    			...
		    	    			//Invalid state
		    	    			console.log("Transfer Recipient not present..from:"+from_address);
		    	    			cb('Transfer Recipient not present');
		    	    		}     		    	
		    	    	});
		    		}		    				    		
		    	});
